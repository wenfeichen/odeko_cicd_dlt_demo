...
```
dbx execute --cluster-name=<name of interactive cluster> --job=odeko_cicd_dlt_demo-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=odeko_cicd_dlt_demo-sample-integration-test --files-only
dbx launch --job=odeko_cicd_dlt_demo-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `odeko_cicd_dlt_demo/__init__.py` file and tag the current code version:
```
